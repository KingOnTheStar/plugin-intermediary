...
func NodeInfoToAnnotation(meta *metav1.ObjectMeta, nodeInfo *types.NodeInfo) error {
	...
	meta.Annotations["HelloWorld"] = kubelet_client.CurrentTopo + string(info)
	//if _, ok := meta.Annotations["node.alpha/DeviceInformation"]; ok {delete(meta.Annotations, "node.alpha/DeviceInformation")}
